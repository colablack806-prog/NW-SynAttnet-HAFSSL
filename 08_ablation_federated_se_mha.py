# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  HAFSSL  — HYBRID ATTENTION FEDERATED SEMI-SUPERVISED LEARNING       ║
# ║  Ablation   : SE → MHA (reversed ordering)                             ║
# ║  Environment : Kaggle                                                   ║
...
CHECKPOINT_DATASET = 'se-mha-check-hafssl'
...
MODEL_NAME        = "SE+MHA-HAFSSL_10clients_ISCX5class"

_alpha_str   = str(non_iid_alpha).replace('.', '')
_folder_name = (
    f'SE+MHA-HAFSSL_10clients_ISCX5class_e{epochs}_b{batch_size}'
...
# ═════════════════════════════════════════════════════════════════════════════
# SEBlock1D + se_mha_attention_block  (reversed-ordering ablation variant)
# ═════════════════════════════════════════════════════════════════════════════
...
#  SE then MHA (reversed ordering)
def se_mha_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM):
    """Reversed-ordering ablation variant: squeeze-and-excitation applied before
    multi-head attention. Contains exactly the same layers as the proposed
    MHA -> SE block in a different order, so the parameter count is identical."""
    input_dim = x.shape[-1]
    se_output = SEBlock1D()(x)
...
def createHAFSSLModel(inp_size, n_classes):
    ...
    x = se_mha_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM)
