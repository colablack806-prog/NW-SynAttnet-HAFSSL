...
Federated extension of SynAttnNet. Clients train the hybrid-attention autoencoder
on unlabeled flows, then fine-tune the classifier on their labeled subset; encoder
and classifier weights are aggregated at the server via federated averaging.
"""
...
def updateServerModel(clientAEWeight, clientCNNWeight):
    ...
    for ind in range(len(clientAEWeight)):
        if firstClientFlag:
            ...
            deepAEModelAggWeights[ind] += clientAEWeight[ind]
            deepCNNModelAggWeights[ind] += clientCNNWeight[ind]
...
# -------2. The training data is split according to the number of sub nodes ----------
    ...
    for clientID in range(numOfClients):
        xClientsList.append(
            xClients[lastLowerBound: lastLowerBound + clientDataInterval])
        yClientsList.append(
            yClients[lastLowerBound: lastLowerBound + clientDataInterval])
