...
# ║  Ablation   : MHA only (multi-head attention, no SE)                   ║
# ║  Environment : Kaggle                                                   ║
...
CHECKPOINT_DATASET = 'mha-check-hafssl'
...
MODEL_NAME        = "MHA-HAFSSL_10clients_ISCX5class"

_alpha_str   = str(non_iid_alpha).replace('.', '')
_folder_name = (
    f'MHA-HAFSSL_10clients_ISCX5class_e{epochs}_b{batch_size}'
...
# ═════════════════════════════════════════════════════════════════════════════
# SEBlock1D + mha_attention_block  (MHA-only ablation variant)
# ═════════════════════════════════════════════════════════════════════════════
...
# attention_block — MHA only, no SEBlock1D
def mha_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM):
    """MHA-only ablation variant: multi-head attention without channel recalibration"""
    input_dim = x.shape[-1]
    attention_output = MultiHeadAttention(
        num_heads=num_heads, key_dim=key_dim, dropout=DROPOUT_RATE,
    )(x, x)
    # no SEBlock1D applied to attention_output
...
def createHAFSSLModel(inp_size, n_classes):
    ...
    x = mha_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM)
