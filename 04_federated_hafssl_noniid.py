# -*- coding: utf-8 -*-
"""HAFSSL - Hybrid Attention-enhanced Federated Semi-Supervised Learning (non-IID)

Federated extension of SynAttnNet under non-IID client partitions. Client data is
partitioned with a Dirichlet distribution (concentration parameter non_iid_alpha),
producing label, quantity and feature skew across clients. Clients train the
hybrid-attention autoencoder on unlabeled flows, then fine-tune the classifier on
their labeled subset; encoder and classifier weights are aggregated at the server
by sample-size-weighted federated averaging.
"""
...
root_path = ''
labelnum = 1000  # The amount of labeled data (per class)
latent_dim = 39
...
os.makedirs("./Models/HAFSSLmodel", exist_ok=True)

monitoring_filename = "./result/HAFSSL_" + str(labelnum) + "label_monitoring.csv"
performance_filename = "./result/HAFSSL_" + str(labelnum) + "label_performance.csv"
AEmodelLocation = "./Models/HAFSSL_AE_" + str(numOfClients) + "_nodes.keras"
CNNmodelLocation = "./Models/HAFSSL_CNN_" + str(numOfClients) + "_nodes.keras"
...
@register_keras_serializable()  # Registering the custom class
class SEBlock1D(Layer):
    """Squeeze-and-Excitation Block for 1D data"""
    def __init__(self, ratio=8, **kwargs):  # Changed ratio from 16 to 8
...
def hybrid_attention_block(x, num_heads=2, key_dim=64):
    """Hybrid attention block: multi-head attention followed by squeeze-and-excitation"""
    attention_output = MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, dropout=0.1)(x, x)  # Changed dropout to 0.25
    attention_output = SEBlock1D()(attention_output)
    x = Add()([x, attention_output])
    x = LayerNormalization(epsilon=1e-6)(x)
    ffn = Dense(key_dim * 4, activation="relu")(x)
    ffn = Dropout(0.1)(ffn)  # Changed dropout to 0.25
...
def createHAFSSLModel(inp_size, n_classes):
    ...
    x = hybrid_attention_block(x, num_heads=2, key_dim=64)
    ...
    cnn = Model(input_c, x, name="synattnnet_classifier")
    cnn.summary()
...
if __name__ == '__main__':
    ...
    gdrive_file_url = 'https://drive.google.com/uc?id=1BOroA02VgM3txOn9ehBDqNHCgtpUHx5I'
    # Public alternative (same file, released by Wang et al.):
    # https://github.com/PrinceXuan12138/HGW-TC-Experimental-code/releases/download/v1.0.0/pcapdroid_10class_each_normalized_cuttedflowfeature.csv


    dataset_path = 'pcapdroid_10class_each_normalized_cuttedflowfeature.csv'
...
# -------2. Create non-IID data distribution across clients ----------
    ...
    for iterationNo in range(1, numOfIterations + 1):
        ...
        for clientID in range(numOfClients):
           ...
           # Store the metrics
            ...
            subCNNmodel.save("./Models/HAFSSLmodel/CNN_node_" +
                             str(clientID) + ".keras")
            subAEmodel.save("./Models/HAFSSLmodel/AE_node_" +
                            str(clientID) + ".keras")
    ...
    for clientID in range(numOfClients):
        ...
        nodemodel.set_weights(load_model(
            "./Models/HAFSSLmodel/CNN_node_" + str(clientID) + ".keras").get_weights())
