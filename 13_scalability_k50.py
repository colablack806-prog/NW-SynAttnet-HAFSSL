...
Scalability experiment: K = 50 clients (Table VIII).

Federated extension of SynAttnNet evaluated at 50 clients under a Dirichlet
non-IID partition. Each client trains the hybrid-attention autoencoder on its
unlabeled flows, briefly fine-tunes the encoder with the classifier head frozen,
then trains the classifier on its labeled subset. Encoder and classifier weights
are aggregated at the server by sample-size-weighted federated averaging (Eq. 5).
...
CHECKPOINT_DATASET = 'checkpoints-50client'
DATASET_PATH       = (
    '/kaggle/input/datasets/XXXXXXXX/iscx-benchmark/'
    'ISCX_5class_each_normalized_cuttedflowfeature.csv'
)
...
# Training parameters
labelnum        = 500   # labeled budget PER CLIENT, split evenly over classes
latent_dim      = 39
verbose         = 2
batch_size      = 256
epochs          = 5
...
MODEL_NAME        = "HAFSSL_K50"

_alpha_str   = str(non_iid_alpha).replace('.', '')
_folder_name = (
    f'HAFSSL_K50_e{epochs}_b{batch_size}'
...
print("=" * 60)
print(f"  {MODEL_NAME} — HAFSSL scalability, ISCX 5 classes  [Kaggle]")
...
def push_checkpoint_to_kaggle(round_num):
    try:
        ...
        if result.returncode == 0:
            # Count client models pushed for verification
...
#
# Keras layer names ("encoder", "decoder", "AEcnn") are deliberately unchanged:
# load_model() resolves them by name, so renaming them would make every existing
# .keras checkpoint — and every model saved by scripts 03–12 — unloadable here.
# ═════════════════════════════════════════════════════════════════════════════
...
def createHAFSSLModel(inp_size, n_classes):
    ...
    # NOTE: the classifier head carries its own hybrid attention block here.
    # Scripts 03/04 do not. See README — this must match before Table VIII is
...
# ═════════════════════════════════════════════════════════════════════════════
# WEIGHTED FEDAVG AGGREGATOR — weights by n_k / n, per Eq. 5
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# VISUALIZATIONS — sized for K = 50
# ═════════════════════════════════════════════════════════════════════════════

def plot_non_iid_distribution(yClientsList, labels, save_path):
    fig, ax = plt.subplots(figsize=(14, max(8, len(yClientsList) * 0.35)))
    ...
    sns.heatmap(
        ...
        xticklabels=labels,
        yticklabels=[f'C{i}' for i in range(len(yClientsList))],
...
def plot_per_client(tracker, save_path):
    # With 50 clients individual lines are unreadable — show mean ± std band.
    fig, ax = plt.subplots(figsize=(10, 6))
    all_accs = np.array([
        [tracker.per_client_acc[r][c] for c in range(numOfClients)]
        for r in tracker.rounds
    ])
    mean_acc = all_accs.mean(axis=1)
    std_acc  = all_accs.std(axis=1)
    min_acc  = all_accs.min(axis=1)
    max_acc  = all_accs.max(axis=1)

    ax.fill_between(tracker.rounds, min_acc, max_acc,
                    alpha=0.15, color='#1976D2', label='Min–Max range')
    ax.fill_between(tracker.rounds,
                    mean_acc - std_acc, mean_acc + std_acc,
                    alpha=0.30, color='#1976D2', label='Mean ± Std')
    ax.plot(tracker.rounds, mean_acc, '-', color='#1976D2',
            linewidth=2, label='Mean client acc')
    ...
    ax.set_title(
        f'{MODEL_NAME} — Per-Client Accuracy Distribution '
...
def plot_final_bars(acc_l, prec_l, rec_l, f1_l, save_path):
    # Horizontal bars — 50 client labels do not fit on a vertical axis.
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 14))
    y   = np.arange(len(acc_l))
    colors_acc = ['#E53935' if a < 0.70 else '#FFA726'
                  if a < 0.85 else '#43A047' for a in acc_l]
    colors_f1  = ['#E53935' if f < 0.70 else '#FFA726'
                  if f < 0.85 else '#43A047' for f in f1_l]

    ax1.barh(y, acc_l, color=colors_acc, edgecolor='white')
    ax1.axvline(x=np.mean(acc_l), color='navy', linestyle='--',
                alpha=0.7, label=f'Mean={np.mean(acc_l):.3f}')
    ax1.set_yticks(y)
    ax1.set_yticklabels([f'C{i}' for i in range(len(acc_l))], fontsize=7)
    ax1.set_xlabel('Accuracy'); ax1.set_title('Final Accuracy', fontweight='bold')
    ax1.set_xlim([0, 1.15]); ax1.legend(fontsize=9)

    ax2.barh(y, f1_l, color=colors_f1, edgecolor='white')
    ax2.axvline(x=np.mean(f1_l), color='navy', linestyle='--',
                alpha=0.7, label=f'Mean={np.mean(f1_l):.3f}')
    ax2.set_yticks(y)
    ax2.set_yticklabels([f'C{i}' for i in range(len(f1_l))], fontsize=7)
    ax2.set_xlabel('F1 Score'); ax2.set_title('Final F1', fontweight='bold')
    ax2.set_xlim([0, 1.15]); ax2.legend(fontsize=9)

    plt.suptitle(f'{MODEL_NAME} — Final Metrics per Client',
...
def plot_dashboard(tracker, acc_list, recon_err, profiles, save_path):
    ...
    order   = np.argsort(acc_arr)
    sorted_acc = acc_arr[order]
    colors_bar = ['#E53935' if a < 0.70 else '#FFA726'
                  if a < 0.85 else '#43A047' for a in sorted_acc]
    ax2.barh(range(len(sorted_acc)), sorted_acc,
             color=colors_bar, edgecolor='white')
    ax2.axvline(x=np.mean(acc_arr), color='navy', linestyle='--',
                alpha=0.6, label=f'Mean={np.mean(acc_arr):.3f}')
    ax2.set_xlabel('Accuracy')
    ax2.set_title(f'Client Acc Distribution (K={numOfClients})',
                  fontweight='bold')
    ax2.set_yticks([])
...
if __name__ == '__main__':
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        print(f"\n  Sample sizes (first 10): {client_sample_sizes[:10]} ...")
    ...
    for cid in range(numOfClients):
        ...
        # Fallback — use global weights if a client file is missing
