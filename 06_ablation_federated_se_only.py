...
# ║  Ablation   : SE only (squeeze-and-excitation, no MHA)                 ║
# ║  Environment : Kaggle                                                   ║
...
DATASET_PATH = (
    '/kaggle/input/datasets/xxxx/iscx-benchmark/ISCX_5class_each_normalized_cuttedfloefeature.csv'
)

CHECKPOINT_DATASET = 'se-check-hafssl'
...
MODEL_NAME        = "SE-HAFSSL_10clients_ISCX5class-new"

_alpha_str   = str(non_iid_alpha).replace('.', '')
_folder_name = (
    f'SE-HAFSSL_10clients_ISCX5class_new_e{epochs}_b{batch_size}'
...
# ═════════════════════════════════════════════════════════════════════════════
# SEBlock1D + se_attention_block  (SE-only ablation variant)
# ═════════════════════════════════════════════════════════════════════════════
...
# B16 — SE only
def se_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM):
    """SE-only ablation variant: squeeze-and-excitation without multi-head attention"""
    input_dim = x.shape[-1]
...
def createHAFSSLModel(inp_size, n_classes):
    ...
    x = se_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM)
