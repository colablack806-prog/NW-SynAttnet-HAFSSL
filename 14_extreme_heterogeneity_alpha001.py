...
Extreme heterogeneity experiment: K = 10 clients, Dirichlet alpha = 0.01
(Table VIII).

Companion to the K = 50 scalability run. Here the client count is held at 10 and
the Dirichlet concentration is pushed to 0.01, so most clients hold flows from a
single service class. Each client trains the hybrid-attention autoencoder on its
...
Environment : Kaggle
Dataset     : ISCX 5-class (service-level classification)
...
LABELS = [
    'Chat', 'Email', 'FileTransfer', 'Streaming', 'VoIP',
]
...
CHECKPOINT_DATASET = '0-01-hafssl-check-point'
CHECKPOINT_INPUT   = (
    f'/kaggle/input/datasets/{KAGGLE_USERNAME}/{CHECKPOINT_DATASET}/'
)

# Training parameters
labelnum        = 1000   # labeled budget PER CLIENT (1000 / 5 = 200 per class)
latent_dim      = 39
verbose         = 2
batch_size      = 256
epochs          = 3
...
MODEL_NAME = "HAFSSL_K10_alpha001"

_alpha_str   = str(non_iid_alpha).replace('.', '')
_folder_name = (
    f'HAFSSL_K10_alpha001_e{epochs}_b{batch_size}'
...
print("=" * 60)
print(f"  {MODEL_NAME} — HAFSSL extreme heterogeneity, ISCX 5 classes [Kaggle]")
...
# .keras checkpoint — and every model saved by the other scripts — unloadable.
# ═════════════════════════════════════════════════════════════════════════════
...
def split_non_iid(data, labels, num_clients, alpha=0.5, min_samples=5):
    ...
    if min_size < min_samples:
        print(f"  ⚠️  Partition retry limit reached at α={alpha}: smallest "
              f"client holds {min_size} samples (target ≥ {min_samples}). "
              f"Clients below the 50-sample floor will be skipped in training.")
...
# ═════════════════════════════════════════════════════════════════════════════
# VISUALIZATIONS — per-client detail, readable at K = 10
# ═════════════════════════════════════════════════════════════════════════════
...
if __name__ == '__main__':
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        print(f"  Dataset: ISCX 5-Class ({len(LABELS)} classes)")
