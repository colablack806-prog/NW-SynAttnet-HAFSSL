# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  HAFSSL v2 + FedProx — HYBRID ATTENTION FEDERATED SEMI-SUPERVISED      ║
# ║  Environment : Kaggle                                                   ║
# ║  Dataset     : ISCX 5-Class (chat / email / file / streaming / voip)   ║
# ║                                                                          ║
# ║  CHANGES vs FedAvg version:                                             ║
# ║  • Stage 2 CNN training uses FedProx proximal penalty                  ║
# ║    L_total = L_CE + (μ/2) * ||w_local - w_global||²                   ║
# ║  • Aggregation remains weighted FedAvg (unchanged)                     ║
# ║  • Stage 1 AE and Stage 1.5 encoder fine-tune unchanged               ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
# ── FedProx parameter ─────────────────────────────────────────────────────
# μ (mu): proximal penalty strength
#   • Higher μ → local model stays closer to global → less client drift
#   • Lower  μ → more local freedom → faster convergence but more drift
#   Recommended ranges:
#     α=0.5  (moderate non-IID) : 0.01 – 0.1
#     α=0.1  (strong non-IID)   : 0.1  – 0.5
#     α=0.01 (extreme non-IID)  : 0.5  – 2.0
FEDPROX_MU = 0.1   # ← tune this value for your non-IID level

# Resume / fresh-start control
FORCE_FRESH_START = False
MODEL_NAME        = "HAFSSL_FedProx"

# ─── Path setup ───────────────────────────────────────────────────────────
_alpha_str   = str(non_iid_alpha).replace('.', '')
_mu_str      = str(FEDPROX_MU).replace('.', '')
_folder_name = (
    f'HAFSSLv2_FedProx_mu{_mu_str}_e{epochs}_b{batch_size}'
...
print(f"  ✅ HYBRID ATTENTION v2 + FedProx")
print(f"  ✅ Fix 1: FFN dimension bug fixed")
print(f"  ✅ Fix 2: Dropout = {DROPOUT_RATE}")
print(f"  ✅ Fix 3: Attention in CNN classifier")
print(f"  ✅ Fix 4: Stage 1.5 supervised encoder fine-tuning")
print(f"  ✅ FedProx μ={FEDPROX_MU}")
print(f"  α={non_iid_alpha} | epochs={epochs} | "
      f"rounds={numOfIterations} | batch={batch_size}")
    ...
    try:
        ...
        result = subprocess.run(
            ['kaggle', 'datasets', 'version',
             '-p', WORKING_BASE,
             '-m', f'[HAFSSLv2-FedProx] Round {round_num}/{numOfIterations} '
                   f'μ={FEDPROX_MU} e={epochs} b={batch_size} α={non_iid_alpha}',
             '--dir-mode', 'zip'],
...
def hybrid_attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM):
    """Hybrid attention: MultiHeadAttention + SEBlock1D + FFN."""
    input_dim = x.shape[-1]
...
# ═════════════════════════════════════════════════════════════════════════════
# ✅ FEDPROX — local training objective with proximal penalty
#
#  Standard FedAvg Stage 2:
#    minimise  L_CE(w)
#
#  FedProx Stage 2:
#    minimise  L_CE(w)  +  (μ/2) * ||w - w_global||²
#
#  The proximal term penalises the local model for drifting too far from
#  the global model, which is the main cause of instability at low α.
#  Only Stage 2 (supervised CNN training) is modified.
#  Stage 1 (AE) and Stage 1.5 (encoder fine-tune) are unchanged because:
#    - Stage 1 is unsupervised reconstruction — drift there does not cause
#      the class-prediction inconsistency FedProx is designed to fix.
#    - Stage 1.5 is frozen-head fine-tuning — it is already constrained.
# ═════════════════════════════════════════════════════════════════════════════

def fedprox_train_step(model, x_batch, y_batch,
                       global_weights, optimizer, mu=FEDPROX_MU):
    """
    Single FedProx gradient step.

    Parameters
    ----------
    model         : local Keras model being trained
    x_batch       : input batch
    y_batch       : label batch (one-hot)
    global_weights: list of global model trainable variable numpy arrays,
                    captured ONCE before local training starts each round
    optimizer     : tf.keras.optimizers instance
    mu            : proximal penalty coefficient (FEDPROX_MU from CONFIG)

    Returns
    -------
    total_loss : float  (CE loss + proximal term)
    ce_loss    : float  (cross-entropy only, for logging)
    """
    with tf.GradientTape() as tape:
        ...
        prox_term = tf.add_n([
            tf.reduce_sum(
                tf.square(w_local - tf.cast(w_global, w_local.dtype)))
            for w_local, w_global in zip(
                model.trainable_variables, global_weights)
        ])
    ...
    return float(total_loss), float(ce_loss)


def fedprox_fit(model, x_data, y_data, global_weights,
                n_epochs, batch_size, val_data=None, verbose=2):
    """
    FedProx training loop — replaces model.fit() for Stage 2.
    Returns a history dict with the same keys as Keras history
    so the rest of the logging code is unchanged.

    Parameters
    ----------
    model          : local Keras model
    x_data         : labeled training features
    y_data         : labeled training labels (one-hot)
    global_weights : snapshot of global model trainable variables
                     taken BEFORE local training starts
    n_epochs       : number of local epochs (= epochs config value)
    batch_size     : mini-batch size
    val_data       : tuple (x_val, y_val) for validation accuracy logging
    verbose        : 0 = silent, 2 = one line per epoch
    """
    ...
    for epoch in range(n_epochs):
        ...
        for x_batch, y_batch in dataset:
            total_loss, ce_loss = fedprox_train_step(
                model, x_batch, y_batch, global_weights, optimizer)
            batch_losses.append(total_loss)

        # Training accuracy on full labeled set
        y_pred    = model.predict(x_data, verbose=0)
        train_acc = accuracy_score(
            np.argmax(y_data, 1), np.argmax(y_pred, 1))
        ...
        if val_data is not None:
            ...
            val_acc = accuracy_score(
                np.argmax(yv, 1), np.argmax(yv_pred, 1))
            hist_val.append(val_acc)
            if verbose == 2:
                print(f"    Epoch {epoch+1}/{n_epochs} — "
                      f"loss:{np.mean(batch_losses):.4f}  "
                      f"acc:{train_acc:.4f}  val_acc:{val_acc:.4f}  "
                      f"[FedProx μ={FEDPROX_MU}]")
        else:
            hist_val.append(0.0)
            if verbose == 2:
                print(f"    Epoch {epoch+1}/{n_epochs} — "
                      f"loss:{np.mean(batch_losses):.4f}  "
                      f"acc:{train_acc:.4f}  [FedProx μ={FEDPROX_MU}]")

    # Return dict matching Keras history format
    return {'accuracy': hist_acc, 'val_accuracy': hist_val}
...
# ═════════════════════════════════════════════════════════════════════════════
# WEIGHTED FEDAVG AGGREGATOR  (unchanged — FedProx only affects local training)
# ═════════════════════════════════════════════════════════════════════════════
...
def plot_non_iid_bar(yClientsList, labels, save_path):
    ...
    for cls in range(len(labels)):
        counts = [np.sum(np.argmax(yClientsList[c], axis=1) == cls)
                  for c in range(len(yClientsList))]
        ax.bar([f'C{i}' for i in range(len(yClientsList))],
...
def plot_convergence(tracker, save_path):
    ...
    ax.set_title(
        ...
        f'(α={non_iid_alpha}, μ={FEDPROX_MU}, e={epochs})',
        fontweight='bold')
...
def plot_per_client(tracker, save_path):
    ...
    ax.set_ylim([0, 1.05]); ax.legend(loc='lower right', fontsize=9)
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches='tight')
    plt.show()
    print(f"  Saved: {save_path}")
...
def plot_final_bars(acc_l, prec_l, rec_l, f1_l, save_path):
    ...
    ax.set_ylabel('Score')
    ax.set_title(f'{MODEL_NAME} — Final Metrics per Client', fontweight='bold')
...
def plot_confusion(y_true, y_pred, labels, save_path):
    ...
    plt.suptitle(
        f'{MODEL_NAME} — Confusion Matrix (α={non_iid_alpha}, μ={FEDPROX_MU})',
...
def plot_dashboard(tracker, acc_list, recon_err, profiles, save_path):
    ...
    colors_bar = ['#E53935' if a < 0.70 else '#FFA726'
                  if a < 0.85 else '#43A047' for a in acc_list]
    ...
    txt = (
        ...
        f"α={non_iid_alpha} | μ={FEDPROX_MU}\n"
        f"Labels={labelnum} ({labelnum//len(LABELS)}/class)\n"
        f"Clients={numOfClients} | Rounds={numOfIterations}\n"
...
if __name__ == '__main__':
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        if start_round == 1:
            print("\n  Creating new model (HAFSSL v2 + FedProx)...")
        ...
        print(f"  α={non_iid_alpha} | μ={FEDPROX_MU} | K={numOfClients} | "
              f"epochs={epochs} | batch={batch_size}")
        ...
        for iterationNo in range(start_round, numOfIterations + 1):
            ...
            for clientID in range(numOfClients):
                ...
                # ── Guard: skip clients with too few samples ──────────────
                ...
                monitor     = Monitor(1, "HAFSSL-FedProx training",
                ...
                # ── Stage 1: AE unsupervised (unchanged) ──────────────────
                ...
                # ── Stage 1.5: supervised encoder fine-tune (unchanged) ───
                ...
                # ── Stage 2: CNN supervised — FedProx ─────────────────────
                # Snapshot global weights BEFORE local training begins.
                # The proximal term penalises deviation from these weights
                # throughout the entire local epoch loop.
                global_cnn_weights = [
                    w.numpy()
                    for w in originclassificationmodel.trainable_variables
                ]

                history_dict = fedprox_fit(
                    subCNNmodel,
                    xClientsListLabel[clientID],
                    yClientsListLabel[clientID],
                    global_weights = global_cnn_weights,
                    n_epochs       = epochs,
                    batch_size     = batch_size,
                    val_data       = (xServer, yServer),
                    verbose        = verbose,
                )
                monitor.stop()
            ...
            # ── Aggregation (weighted FedAvg — unchanged) ─────────────────
    ...
    for cid in range(numOfClients):
        ...
        if os.path.exists(cnn_path):
            ...
            print(f"  Client {cid} — loaded from saved model")
        else:
            print(f"  ⚠️  Client {cid} — model file missing, "
                  f"using global model weights")
    ...
    print(f"  FedProx μ={FEDPROX_MU} | α={non_iid_alpha}")
